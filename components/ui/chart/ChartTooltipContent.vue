...
const payload = computed(() => {
  return Object.entries(props.payload).filter(([key]) => !props.hiddenKeys?.includes(key)).map(([key, value]) => {
...
const nestLabel = computed(() => payload.value.length === 1 && props.indicator !== "dot")
